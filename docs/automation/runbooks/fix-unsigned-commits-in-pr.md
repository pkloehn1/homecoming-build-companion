...
python -m scripts.github.fix_unsigned_commits --pr <NUMBER>
...
python -m scripts.github.list_pr_commit_verifications --repo owner/name --pr <NUMBER> --only-failing
...
```bash
python -m scripts.github.list_pr_commit_verifications \
  --repo owner/name \
  --pr <PR_NUMBER> \
  --only-failing
```
