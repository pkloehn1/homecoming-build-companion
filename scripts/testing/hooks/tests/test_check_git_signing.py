...
def _clear_skip_env(monkeypatch: Any) -> None:
    monkeypatch.delenv("SKIP_GIT_SIGNING_CHECK", raising=False)


def _patch_home(monkeypatch: Any, tmp_path: Path) -> None:
...
def test_main_skip_env(monkeypatch: Any) -> None:
    """Test main skip env."""
...
def test_main_configured(monkeypatch: Any, tmp_path: Path) -> None:
    """Test main configured."""
...
def test_main_missing_key(monkeypatch: Any, tmp_path: Path) -> None:
    """Test main missing key."""
    _clear_skip_env(monkeypatch)
    _patch_config(monkeypatch)
    (tmp_path / ".ssh").mkdir()
    _patch_home(monkeypatch, tmp_path)
...
def test_main_not_configured(monkeypatch: Any, tmp_path: Path) -> None:
...
def test_test_signing_success(tmp_path: Path) -> None:
    ...
    with patch("scripts.testing.hooks.check_git_signing.subprocess.run", return_value=completed) as mock_run:
...
def test_test_signing_failure(tmp_path: Path) -> None:
    ...
    with patch("scripts.testing.hooks.check_git_signing.subprocess.run", return_value=completed):
        sign_ok, msg = mod._test_signing(fake_key)
...
def test_test_signing_timeout(tmp_path: Path) -> None:
    ...
    fake_key.write_text("fake")
    with patch(
        "scripts.testing.hooks.check_git_signing.subprocess.run",
...
def test_test_signing_missing_ssh_keygen(tmp_path: Path) -> None:
    ...
    fake_key = tmp_path / "key"
    fake_key.write_text("fake")
    with patch("scripts.testing.hooks.check_git_signing.subprocess.run", side_effect=FileNotFoundError("ssh-keygen")):
        sign_ok, msg = mod._test_signing(fake_key)
    assert sign_ok is False
    assert "ssh-keygen not found" in msg


def test_main_fails_when_signing_smoke_test_fails(monkeypatch: Any, tmp_path: Path) -> None:
...
def test_check_passes_when_gpg_ssh_program_configured(monkeypatch: Any, tmp_path: Path) -> None:
    """When gpg.ssh.program is set and exists, skip ssh-keygen smoke test."""
    _clear_skip_env(monkeypatch)
    _patch_config_with_ssh_program(monkeypatch)
    ssh_dir = tmp_path / ".ssh"
    ssh_dir.mkdir()
    key_path = ssh_dir / "id_ed25519_signing.pub"
    key_path.write_text("ssh-ed25519 AAAA test\n")
    _patch_home(monkeypatch, tmp_path)
    # Create a fake executable where gpg.ssh.program points
...
def test_check_wrong_gpg_format(monkeypatch: Any) -> None:
    """Test check wrong gpg format."""
    _clear_skip_env(monkeypatch)
...
def test_check_missing_signingkey(monkeypatch: Any) -> None:
    """Test check missing signingkey."""
    _clear_skip_env(monkeypatch)
...
def test_check_git_config_raises(monkeypatch: Any) -> None:
    """Test check git config raises."""
    _clear_skip_env(monkeypatch)
...
def test_check_fails_when_gpg_ssh_program_missing(monkeypatch: Any, tmp_path: Path) -> None:
    ...
    key_path.write_text("ssh-ed25519 AAAA test\n")
    _patch_home(monkeypatch, tmp_path)
    monkeypatch.setattr("scripts.testing.hooks.check_git_signing.shutil.which", lambda _prog: None)
    signing_ok, _msg = mod.check_git_signing()
    assert signing_ok is False


def test_check_passes_when_gpg_ssh_program_on_path(monkeypatch: Any, tmp_path: Path) -> None:
    ...
    _patch_home(monkeypatch, tmp_path)
    monkeypatch.setattr(
        "scripts.testing.hooks.check_git_signing.shutil.which",
        lambda _prog: "/usr/bin/op-ssh-sign",
    )
    signing_ok, _msg = mod.check_git_signing()
    assert signing_ok is True
