#!/usr/bin/env python3
"""Run Super-Linter locally against the same image and config as CI.

Runs the pinned Super-Linter image as a one-shot container so local linting
matches the ``.github/workflows/super-linter.yml`` job (local == remote).  The
shared ``VALIDATE_*`` flags come from ``.github/super-linter.env`` via Docker's
``--env-file``; only environment-specific overrides (workspace mount path,
default branch, dev-artifact excludes) are passed as ``-e`` args here.

Execution model:
- Linux/macOS/Windows: ``docker run --rm`` one-shot container.
...
- SUPER_LINTER_DEFAULT_BRANCH: Override the diff base branch (default: origin/HEAD or main).
- SUPER_LINTER_VALIDATE_ALL_CODEBASE: Force full-codebase scan ("true"/"false").
- FILTER_REGEX_EXCLUDE: Override the default local exclude pattern.
"""
...
def _maybe_append_default_filter_excludes(env_pairs: list[tuple[str, str]], *, image: str) -> None:
    # Keep local == remote: mirror the CI FILTER_REGEX_EXCLUDE from
    # .github/workflows/super-linter.yml, plus dev-only artifacts that only
    # exist locally (virtualenv, tool caches).  Super-Linter's JSCPD/other
    # validators would otherwise scan .venv/, producing noisy, slow failures
    # that never happen in CI.
    #
    ...
    # Dev-only artifacts (local machine only).
    dev_excluded_dirs = (
        r"\.venv",
        r"\.tox",
        r"\.pytest_cache",
    ...
    dev_pattern = r"(^|/)({})/".format("|".join(dev_excluded_dirs))

    # CI-parity exclusions: verbatim/harvested game data, generated extraction
    # docs, and the legacy PowerShell capture lane.  Mirrors the pattern in
    # .github/workflows/super-linter.yml (keep the two in sync).
    ci_pattern = (
        r"((.*/)?(samples/|src/coh_engine/tests/fixtures/|data/canonical/|docs/engine/).*"
        r"|scripts/[^/]+\.ps1|scripts/lane-[ab]/.*)"
    )

    env_pairs.append(("FILTER_REGEX_EXCLUDE", f"{dev_pattern}|{ci_pattern}"))
...
SUPER_LINTER_ENV_FILE = ".github/super-linter.env"


def _build_env_pairs(repo_root: Path, *, image: str, default_branch: str) -> list[tuple[str, str]]:
    # Shared VALIDATE_* flags and non-path settings load from
    # .github/super-linter.env via Docker --env-file.  This list holds only the
    # environment-specific overrides that differ between local and CI (workspace
    # mount path, default branch, full-scan decision).
...
def _build_docker_args(
    ...
    *,
    ...
    # Load shared linter config from .github/super-linter.env (SSOT for
    # VALIDATE_* flags).  Environment-specific overrides in env_pairs below.
    env_file = repo_root / SUPER_LINTER_ENV_FILE
    if env_file.exists():
