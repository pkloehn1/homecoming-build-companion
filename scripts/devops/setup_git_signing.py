...
Usage:
    python -m scripts.devops.setup_git_signing [--check-only] [--key-path PATH] [--email EMAIL] [--passphrase PASSPHRASE | --no-passphrase]
...
def _print_display_key_instruction(key_path: Path, *, platform: str | None = None) -> None:
...
def print_next_steps(key_path: Path, *, platform: str | None = None) -> None:
    """Print instructions for next steps."""
    print("\n[INFO] Next steps:")
    _print_ssh_add_instruction(key_path)
    _print_github_registration_instruction(key_path)
    _print_display_key_instruction(key_path, platform=platform)
