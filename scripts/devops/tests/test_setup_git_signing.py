...
# Test fixture passphrase — not a real credential.
_TEST_PASSPHRASE = "test-passphrase"


def test_find_existing_signing_key_prefers_existing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Test find existing signing key prefers existing."""
...
def test_check_only_mode(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_configure_git_signing_skips_when_already_configured(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Test configure git signing skips when already configured."""
    ssh_dir = tmp_path / ".ssh"
    ssh_dir.mkdir()
    ...
    configure_git_spy = MagicMock()
    monkeypatch.setattr(mod, "_configure_git", configure_git_spy)
    monkeypatch.setattr(
        mod,
        "check_git_config",
        lambda: {
    ...
    configure_git_spy.assert_not_called()


def test_email_from_gh_payload_prefers_email() -> None:
    """Test email from gh payload prefers email."""
...
def test_email_from_gh_payload_uses_noreply_when_missing_email() -> None:
    """Test email from gh payload uses noreply when missing email."""
    payload = {"email": None, "login": "octocat"}
    assert mod._email_from_gh_payload(payload) == "octocat@users.noreply.github.com"


def test_email_from_git_config_accepts_valid_email(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test email from git config accepts valid email."""
    monkeypatch.setattr(mod, "git_config_value", lambda _key: "dev@example.com")
    assert mod._email_from_git_config() == "dev@example.com"


def test_email_from_git_config_rejects_invalid_email(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test email from git config rejects invalid email."""
...
def test_resolve_signing_key_linux_uses_private_key(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_resolve_signing_key_linux_fails_without_private_key(
...
def test_resolve_signing_key_windows_uses_public_key(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_resolve_signing_key_windows_fails_without_public_key(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
...
def test_get_ssh_dir(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Test get ssh dir."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_split_key_paths_with_pub_suffix(tmp_path: Path) -> None:
    """Test split key paths with pub suffix."""
...
def test_split_key_paths_without_pub_suffix(tmp_path: Path) -> None:
    """Test split key paths without pub suffix."""
...
def test_looks_like_private_key_true(tmp_path: Path) -> None:
    """Test looks like private key true."""
...
def test_looks_like_private_key_false(tmp_path: Path) -> None:
    """Test looks like private key false."""
...
def test_looks_like_private_key_missing_file(tmp_path: Path) -> None:
    """Test looks like private key missing file."""
...
def test_regenerate_public_key_success(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test regenerate public key success."""
...
def test_regenerate_public_key_called_process_error(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test regenerate public key called process error."""
    private_key = tmp_path / "id_ed25519"
    public_key = tmp_path / "id_ed25519.pub"
...
def test_regenerate_public_key_called_process_error_no_stderr(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Test regenerate public key called process error no stderr."""
    private_key = tmp_path / "id_ed25519"
    public_key = tmp_path / "id_ed25519.pub"
...
def test_regenerate_public_key_not_found(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test regenerate public key not found."""
    private_key = tmp_path / "id_ed25519"
    public_key = tmp_path / "id_ed25519.pub"
...
def test_generate_signing_key_success(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test generate signing key success."""
    private_key = tmp_path / "id_ed25519_signing"

    with patch("subprocess.run", return_value=MagicMock()) as mock_run:
        result = mod._generate_signing_key(private_key, passphrase=_TEST_PASSPHRASE)
...
def test_generate_signing_key_called_process_error(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test generate signing key called process error."""
    private_key = tmp_path / "id_ed25519_signing"
...
def test_generate_signing_key_called_process_error_no_stderr(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Test generate signing key called process error no stderr."""
    private_key = tmp_path / "id_ed25519_signing"
...
def test_generate_signing_key_not_found(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test generate signing key not found."""
    private_key = tmp_path / "id_ed25519_signing"
...
def test_create_signing_key_already_exists(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test create signing key already exists."""
    key = tmp_path / "id_ed25519_signing.pub"
    key.write_text("ssh-ed25519 AAAA test\n")
    result = mod.create_signing_key(key, passphrase=_TEST_PASSPHRASE)
    assert result is True
    assert "Key already exists" in capsys.readouterr().out


def test_create_signing_key_private_exists_no_pub(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = mod.create_signing_key(pub_key, passphrase=_TEST_PASSPHRASE)
    assert result is True


def test_create_signing_key_generates_new(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = mod.create_signing_key(private_key, passphrase=_TEST_PASSPHRASE)
    assert result is True
...
def test_email_from_git_config_returns_none_on_exception(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test email from git config returns none on exception."""
...
def test_email_from_git_config_returns_none_for_empty(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test email from git config returns none for empty."""
    monkeypatch.setattr(mod, "git_config_value", lambda _key: "")
    assert mod._email_from_git_config() is None


def test_email_from_git_config_accepts_noreply(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test email from git config accepts noreply."""
...
def test_email_from_gh_cli_success(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test email from gh cli success."""
    fake_result = MagicMock()
...
def test_email_from_gh_cli_nonzero_returncode(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test email from gh cli nonzero returncode."""
    fake_result = MagicMock()
...
def test_email_from_gh_cli_exception(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test email from gh cli exception."""
...
def test_email_from_gh_cli_invalid_json(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test email from gh cli invalid json."""
    fake_result = MagicMock()
    fake_result.returncode = 0
...
def test_email_from_gh_cli_non_dict_json(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test email from gh cli non dict json."""
    fake_result = MagicMock()
    fake_result.returncode = 0
...
def test_get_github_email_from_git_config(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test get github email from git config."""
...
def test_get_github_email_falls_back_to_gh_cli(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test get github email falls back to gh cli."""
...
def test_check_git_config_returns_values(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test check git config returns values."""
...
def test_check_git_config_handles_exceptions(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test check git config handles exceptions."""
...
def test_configure_git_success(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Test configure git success."""
    monkeypatch.setattr(mod, "is_windows", lambda: False)
    private_key = tmp_path / "id_ed25519_signing"
...
def test_configure_git_fails_when_key_missing(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Test configure git fails when key missing."""
    monkeypatch.setattr(mod, "is_windows", lambda: False)
    public_key = tmp_path / "id_ed25519_signing.pub"
    public_key.write_text("ssh-ed25519 AAAA test\n")
...
def test_configure_git_called_process_error(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Test configure git called process error."""
    monkeypatch.setattr(mod, "is_windows", lambda: False)
    private_key = tmp_path / "id_ed25519_signing"
    private_key.write_text("fake key content")
    public_key = tmp_path / "id_ed25519_signing.pub"
...
def test_configure_allowed_signers_no_email(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_configure_allowed_signers_missing_public_key(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test configure allowed signers missing public key."""
    pub = tmp_path / "id_ed25519.pub"
    # pub not created
...
def test_configure_allowed_signers_malformed_key(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test configure allowed signers malformed key."""
    pub = tmp_path / "id_ed25519.pub"
...
def test_configure_allowed_signers_success(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test configure allowed signers success."""
    pub = tmp_path / "id_ed25519.pub"
...
def test_configure_allowed_signers_exception(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test configure allowed signers exception."""
    pub = tmp_path / "id_ed25519.pub"
...
def test_configure_git_signing_force_reconfigures(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_configure_git_signing_configure_fails(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Test configure git signing configure fails."""
...
def test_configure_git_signing_already_configured_key_missing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_verify_key_exists_found(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test verify key exists found."""
...
def test_verify_key_exists_not_found(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test verify key exists not found."""
...
def test_verify_git_config_all_correct(capsys: pytest.CaptureFixture[str]) -> None:
    """Test verify git config all correct."""
    config = {
        mod.GIT_CONFIG_COMMIT_GPGSIGN: "true",
        mod.GIT_CONFIG_GPG_FORMAT: "ssh",
...
def test_verify_git_config_gpgsign_wrong(capsys: pytest.CaptureFixture[str]) -> None:
    """Test verify git config gpgsign wrong."""
    config = {
...
def test_verify_git_config_format_wrong(capsys: pytest.CaptureFixture[str]) -> None:
    """Test verify git config format wrong."""
    config = {
        mod.GIT_CONFIG_COMMIT_GPGSIGN: "true",
...
def test_verify_git_config_no_signing_key(capsys: pytest.CaptureFixture[str]) -> None:
    """Test verify git config no signing key."""
    config = {
        mod.GIT_CONFIG_COMMIT_GPGSIGN: "true",
        mod.GIT_CONFIG_GPG_FORMAT: "ssh",
...
def test_verify_signature_capability_good_signature(capsys: pytest.CaptureFixture[str]) -> None:
    """Test verify signature capability good signature."""
    fake_result = MagicMock()
...
def test_verify_signature_capability_no_signature(capsys: pytest.CaptureFixture[str]) -> None:
    """Test verify signature capability no signature."""
    fake_result = MagicMock()
...
def test_verify_setup_key_missing(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Test verify setup key missing."""
    monkeypatch.setattr(mod, "is_windows", lambda: False)
    pub = tmp_path / "id_ed25519_signing.pub"
    pub.write_text("ssh-ed25519 AAAA test\n")
...
def test_verify_setup_bad_config(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Test verify setup bad config."""
    monkeypatch.setattr(mod, "is_windows", lambda: False)
    private_key = tmp_path / "id_ed25519_signing"
    private_key.write_text("fake key content")
    pub = tmp_path / "id_ed25519_signing.pub"
...
def test_print_ssh_add_instruction(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test print ssh add instruction."""
    pub = tmp_path / "id_ed25519_signing.pub"
...
def test_print_github_registration_instruction(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test print github registration instruction."""
    pub = tmp_path / "id_ed25519_signing.pub"
...
def test_print_display_key_instruction_non_windows(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test print display key instruction non windows."""
    pub = tmp_path / "id_ed25519_signing.pub"
    mod._print_display_key_instruction(pub, platform="linux")
    out = capsys.readouterr().out
    assert "cat" in out
    assert "Get-Content" not in out


def test_print_display_key_instruction_windows(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test print display key instruction windows."""
    pub = tmp_path / "id_ed25519_signing.pub"
    mod._print_display_key_instruction(pub, platform="win32")
    out = capsys.readouterr().out
    assert "Get-Content" in out


def test_print_next_steps(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test print next steps."""
    pub = tmp_path / "id_ed25519_signing.pub"
    mod.print_next_steps(pub, platform="linux")
    out = capsys.readouterr().out
...
def test_resolve_key_path_explicit(tmp_path: Path) -> None:
    """Test resolve key path explicit."""
    explicit = tmp_path / "mykey.pub"
    result = mod._resolve_key_path(explicit)
    assert result == explicit.expanduser().resolve()


def test_resolve_key_path_finds_existing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Test resolve key path finds existing."""
...
def test_resolve_key_path_defaults_to_new(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Test resolve key path defaults to new."""
...
def test_handle_check_only_false() -> None:
    """Test handle check only false."""
    assert mod._handle_check_only(False, Path("dummy")) is False


def test_handle_check_only_true_key_exists(
    tmp_path: Path, capsys: pytest.CaptureFixture[str], monkeypatch: pytest.MonkeyPatch
) -> None:
    """Test handle check only true key exists."""
    key = tmp_path / "id_ed25519_signing.pub"
    key.write_text("ssh-ed25519 AAAA test\n")
...
def test_handle_check_only_true_key_missing(
    tmp_path: Path, capsys: pytest.CaptureFixture[str], monkeypatch: pytest.MonkeyPatch
) -> None:
    """Test handle check only true key missing."""
    key = tmp_path / "missing.pub"
    monkeypatch.setattr(mod, "check_git_config", lambda: {mod.GIT_CONFIG_COMMIT_GPGSIGN: "true"})
    result = mod._handle_check_only(True, key)
    assert result is True
...
def test_resolve_email_explicit(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test resolve email explicit."""
...
def test_resolve_email_falls_back_to_github(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test resolve email falls back to github."""
    monkeypatch.setattr(mod, "get_github_email", lambda: "detected@example.com")
    assert mod._resolve_email(None) == "detected@example.com"


def test_resolve_email_warns_when_missing(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    """Test resolve email warns when missing."""
...
def test_ensure_signing_key_exists(tmp_path: Path) -> None:
    """Test ensure signing key exists."""
    key = tmp_path / "id_ed25519_signing.pub"
    key.write_text("ssh-ed25519 AAAA test\n")
    assert mod._ensure_signing_key(key, passphrase=_TEST_PASSPHRASE) is True


def test_ensure_signing_key_no_passphrase(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Test ensure signing key no passphrase."""
    key = tmp_path / "id_ed25519_signing.pub"
    # key not created
...
def test_ensure_signing_key_creates_new(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Test ensure signing key creates new."""
    key = tmp_path / "id_ed25519_signing.pub"
    # key not created
    monkeypatch.setattr(mod, "create_signing_key", lambda _path, passphrase: True)
    result = mod._ensure_signing_key(key, passphrase=_TEST_PASSPHRASE)
    assert result is True
...
def test_resolve_passphrase_no_passphrase_flag() -> None:
    """Test resolve passphrase no passphrase flag."""
...
def test_resolve_passphrase_provided() -> None:
    """Test resolve passphrase provided."""

    class FakeArgs(argparse.Namespace):
        no_passphrase = False
        passphrase = _TEST_PASSPHRASE

    assert mod._resolve_passphrase(FakeArgs()) == _TEST_PASSPHRASE


def test_resolve_passphrase_missing(capsys: pytest.CaptureFixture[str]) -> None:
    """Test resolve passphrase missing."""
...
def test_main_returns_0_when_check_only(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Test main returns 0 when check only."""
...
def test_main_fails_passphrase_required(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Test main fails passphrase required."""
    key = tmp_path / "id_ed25519_signing.pub"
...
def test_main_ensure_signing_key_fails(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Test main ensure signing key fails."""
    key = tmp_path / "id_ed25519_signing.pub"
    ...
    monkeypatch.setattr(mod, "_resolve_key_path", lambda _: key)
    monkeypatch.setattr(mod, "_handle_check_only", lambda _cfg, _key: False)
    monkeypatch.setattr(mod, "_resolve_passphrase", lambda _args: _TEST_PASSPHRASE)
    monkeypatch.setattr(mod, "_resolve_email", lambda _: "user@example.com")
...
def test_main_configure_fails(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Test main configure fails."""
    key = tmp_path / "id_ed25519_signing.pub"
    key.write_text("ssh-ed25519 AAAA test\n")
...
def test_main_verify_fails(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Test main verify fails."""
    key = tmp_path / "id_ed25519_signing.pub"
    key.write_text("ssh-ed25519 AAAA test\n")
